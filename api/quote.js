export default async function handler(req, res) {
  ...
  try {
    const body = req.body || await new Promise((resolve, reject) => { let d=''; req.on('data', c=>d+=c); req.on('end', ()=>{ try{ resolve(JSON.parse(d||'{}')) }catch(e){ reject(e) } }) })
    const { material, finish, lead, qty, metrics } = body || {};
    if (!material || !finish || !lead || !qty || !metrics) { res.status(400).json({ error: 'Missing fields' }); return; }

    const LABOR_RATE=45, MACHINE_RATE=18, OVERHEAD_PCT=0.12, MARGIN_PCT=0.25, PRINT_SPEED_MM_PER_HR=25;
    const MATERIALS={ rigid10k:{cost_per_cc:0.22,handling_factor:1.05}, tough1500:{cost_per_cc:0.18,handling_factor:1.0}, high_temp:{cost_per_cc:0.30,handling_factor:1.15}, esd:{cost_per_cc:0.35,handling_factor:1.25} };
    const FINISH={ basic:{coef:0.003}, cosmetic:{coef:0.008}, paint_ready:{coef:0.015} };
    const LEAD={ standard:{days:3,mult:1.0}, rush:{days:2,mult:1.25}, expedite:{days:1,mult:1.6} };

    const mat=MATERIALS[material], fin=FINISH[finish], lt=LEAD[lead];
    if(!mat||!fin||!lt){ res.status(400).json({ error:'Invalid options' }); return; }
